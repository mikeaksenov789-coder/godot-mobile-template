...
- `hud_layer.gd` (+ `.tscn`) — the HUD root (`CanvasLayer`): hosts
  `SafeArea`, the pause button, and lazily instantiates the Settings/Pause
  screens on demand. `content_root` (`SafeArea/Content`) is intentionally
  empty — a real game's HUD content goes there; nothing gameplay-specific
